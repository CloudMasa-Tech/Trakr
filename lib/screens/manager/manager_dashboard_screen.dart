...
class _ManagerDashboardScreenState extends State<ManagerDashboardScreen> {
  ...
  int _teamRequestSubTab = 0; // 0 = Leave, 1 = Permission
    ...
    return Column(
      ...
      children: [
        ...
        StreamBuilder<Map<String, int>>(
          stream:
              _leaveService.getApprovalStatsByManager(_attendanceManagerScope),
          builder: (context, snapshot) {
            final stats = snapshot.data ??
                {'pending': 0, 'approved': 0, 'approvedToday': 0};
        ...
        StreamBuilder<List<LeaveRequest>>(
          stream: _leaveService.getRequestsForManager(_attendanceManagerScope),
          builder: (context, snapshot) {
            if (snapshot.hasError) {
              debugPrint('❌ UI ERROR: Leave Stream failed: ${snapshot.error}');
              return Center(
                child: Padding(
            ...
            debugPrint(
                '📊 UI DEBUG: Leave requests for "$_attendanceManagerScope": ${requests.length}');
            ...
            if (requests.isEmpty) {
              return Container(
                ...
                child: Center(
                  child: Text(
                    'No leave requests found for your staff members.',
                    style: TextStyle(
                      color: _ManagerDashTheme.muted.withValues(alpha: 0.85),
                      fontSize: isMobile ? 13 : 14,
                    ),
                    textAlign: TextAlign.center,
                  ),
  ...
  Widget _buildPermissionRequestPage() {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'Review and manage immediate permission requests from your staff.',
          style: TextStyle(
            color: _ManagerDashTheme.muted,
            fontSize: isMobile ? 13 : 14,
            height: 1.6,
        ...
        StreamBuilder<Map<String, int>>(
          stream: PermissionService()
              .getApprovalStatsByManager(_attendanceManagerScope),
          builder: (context, statsSnapshot) {
            if (statsSnapshot.hasError) {
              return Container(
                ...
                child: Text(
                  'Error loading stats: ${statsSnapshot.error}',
                  style: const TextStyle(color: Colors.red),
                ),
              );
            }
            ...
            return Row(
              children: [
                ...
                const SizedBox(width: 12),
                _buildStatCard('Rejected', stats['rejected'] ?? 0,
        ...
        StreamBuilder<List<PermissionRequest>>(
          stream: PermissionService()
              .getPermissionRequestsForManager(_attendanceManagerScope),
          builder: (context, snapshot) {
            if (snapshot.hasError) {
              return Container(
                padding: const EdgeInsets.all(16),
            ...
            if (requests.isEmpty) {
              return Container(
                ...
                child: Center(
                  child: Text(
                    'No permission requests found for your staff members.',
                    style: TextStyle(
                      color: _ManagerDashTheme.muted.withValues(alpha: 0.85),
                      fontSize: isMobile ? 13 : 14,
                    ),
            ...
            return Column(
              children: requests
                  .map((request) => _buildPermissionRequestCard(request))
                  .toList(),
            );
          },
        ),
  ...
  Widget _buildTeamRequestsPage() {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'Review and manage staff leave and permission requests in one place.',
          style: TextStyle(
            color: _ManagerDashTheme.muted,
            fontSize: isMobile ? 13 : 14,
            height: 1.6,
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              _requestTabButton('Leave Requests', 0, expand: isMobile),
              _requestTabButton('Permission Requests', 1, expand: isMobile),
            ],
          ),
        ),
        const SizedBox(height: 20),
        _teamRequestSubTab == 0
  ...
  Widget _requestTabButton(String label, int index, {bool expand = false}) {
    final selected = _teamRequestSubTab == index;
    final button = InkWell(
      onTap: () => setState(() => _teamRequestSubTab = index),
      borderRadius: BorderRadius.circular(10),
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected
              ? _ManagerDashTheme.primary.withValues(alpha: 0.16)
              : Colors.transparent,
          borderRadius: BorderRadius.circular(10),
        ),
        child: Text(
          ...
          style: TextStyle(
            color:
                selected ? _ManagerDashTheme.primary : _ManagerDashTheme.muted,
            fontSize: 13,
  ...
  Widget _buildRequestCard(LeaveRequest request) {
    ...
    return FutureBuilder<LeaveBalanceSummary>(
      future: _leaveService.getLeaveBalanceSummaryOnce(request.userId),
      builder: (context, snapshot) {
        ...
        return Container(
          ...
          padding: EdgeInsets.symmetric(
              horizontal: isMobile ? 14 : 20, vertical: isMobile ? 12 : 16),
          decoration: BoxDecoration(
            color: _ManagerDashTheme.surface,
            ...
            children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          request.employeeId?.trim().isNotEmpty == true
                              ? request.employeeId!
                              : request.userId,
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            color:
                                _ManagerDashTheme.muted.withValues(alpha: 0.85),
              ...
              if (isMobile)
                Column(
                  ...
                  children: [
                    _leaveMetaBlock(
                      'Leave Dates',
                      _formatLeaveDates(request),
                    ),
                    const SizedBox(height: 10),
                    _leaveMetaBlock(
                      'Applied Time',
                      _formatIndianAppliedTime(request.createdAt),
                    ),
                    const SizedBox(height: 10),
                    _leaveMetaBlock(
                      'Reason',
              ...
              Wrap(
                ...
                children: [
                  _leaveCountChip(
                    'Total',
                    _leaveService.formatLeaveDays(balance.monthlyAllowance),
                  ...
                  _leaveCountChip(
                    ...
                    _leaveService.formatLeaveDays(
                        balance.approvedDays + balance.pendingDays),
                    const Color(0xFFFFF4D8),
                    _ManagerDashTheme.warning,
                  ),
                  _leaveCountChip(
                    'Left',
                    _leaveService.formatLeaveDays(balance.remainingDays),
                  ...
                  if (request.status == 'pending') ...[
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () => _approveLeaveRequest(request),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: const Color(0xFF24B463),
                          foregroundColor: Colors.white,
                          elevation: 0,
                          padding: EdgeInsets.symmetric(
                    ...
                    SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () => _rejectLeaveRequest(request),
  ...
  Future<void> _approveLeaveRequest(LeaveRequest request) async {
    try {
      await _leaveService.updateLeaveStatus(
        request.id,
        'approved',
        managerResponseReason: 'Approved from manager dashboard',
        actingManagerName: _managerDisplayName,
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _rejectLeaveRequest(LeaveRequest request) async {
    try {
      await _leaveService.updateLeaveStatus(
        request.id,
        'rejected',
        rejectionReason: 'Rejected from manager dashboard',
        actingManagerName: _managerDisplayName,
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showLeaveActionError(Object error) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
  ...
  Future<void> _approvePermissionRequest(PermissionRequest request) async {
    try {
      await PermissionService().approvePermissionRequest(
        requestId: request.id,
        managerName: _managerDisplayName,
        response: 'Approved from manager dashboard',
  ...
  Future<void> _rejectPermissionRequest(PermissionRequest request) async {
    try {
      await PermissionService().rejectPermissionRequest(
        request.id,
        _managerDisplayName,
        'Rejected from manager dashboard',
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildPermissionRequestCard(PermissionRequest request) {
    final isMobile = MediaQuery.of(context).size.width < 800;
    return Container(
      margin: const EdgeInsets.only(bottom: 16),
      ...
      child: Column(
        ...
        children: [
          ...
          if (request.fromTime != null && request.toTime != null)
            _infoPill(
                _formatPermissionWindow(request), _ManagerDashTheme.primary)
          else if (request.returnTime != null)
            _infoPill(
              'Return: ${DateFormat('hh:mm a').format(request.returnTime!)}',
          ...
          if (request.status == 'pending') ...[
            ...
            Row(
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () => _approvePermissionRequest(request),
                    icon: const Icon(Icons.check, size: 18),
                    label: const Text('Approve'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _ManagerDashTheme.success,
                      foregroundColor: Colors.white,
                      elevation: 0,
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () => _rejectPermissionRequest(request),
                    icon: const Icon(Icons.close, size: 18),
                    label: const Text('Reject'),
                    style: OutlinedButton.styleFrom(
                      side: const BorderSide(color: _ManagerDashTheme.danger),
                      foregroundColor: _ManagerDashTheme.danger,
                      padding: const EdgeInsets.symmetric(vertical: 12),
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(8),
                      ),
  ...
  Widget _leaveMetaBlock(String label, String value) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
          label,
          style: TextStyle(
            color: _ManagerDashTheme.muted.withValues(alpha: 0.9),
            fontSize: 12,
          ),
        ),
        const SizedBox(height: 4),
  ...
  String _formatLeaveDates(LeaveRequest request) {
    ...
    if (sameDay) {
      return DateFormat('MMM dd, yyyy').format(request.startDate);
    }
    return '${DateFormat('MMM dd').format(request.startDate)} — '
  ...
  String _formatPermissionWindow(PermissionRequest request) {
    final from = request.fromTime;
    final to = request.toTime;
    if (from == null || to == null) return '';
    final sameDay =
        from.year == to.year && from.month == to.month && from.day == to.day;
    if (sameDay) {
      return '${DateFormat('MMM dd').format(from)}, '
          '${DateFormat('hh:mm a').format(from)} - ${DateFormat('hh:mm a').format(to)}';
    }
    return '${DateFormat('MMM dd, hh:mm a').format(from)} - '
        '${DateFormat('MMM dd, hh:mm a').format(to)}';
  }

  String _formatIndianAppliedTime(DateTime? dt) {
    if (dt == null) return 'N/A';
    final indianTime = dt.toUtc().add(const Duration(hours: 5, minutes: 30));

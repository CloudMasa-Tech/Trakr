...
export const deployTenantRules = onCall(
  {
  ...
  async (request) => {
    ...
    const tenantProjectId = request.data?.tenantProjectId as string | undefined;
    ...
    if (!request.auth?.token?.super_admin) {
      logger.warn('deployTenantRules: permission denied - not superadmin', {
      ...
      throw new HttpsError(
        'permission-denied',
        'Only superadmins can deploy tenant rules. Missing super_admin custom claim.'
      );
    }
